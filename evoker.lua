...
function ConRO:EnableRotationModule(mode)
	...
	if mode == 1 then
		...
		if ConRO.db.profile._Spec_1_Enabled then			
			self.NextSpell = ConRO.Evoker.Havoc;
			self.ToggleDamage();
	...
	if mode == 2 then
		...
		if ConRO.db.profile._Spec_2_Enabled then			
			self.NextSpell = ConRO.Evoker.Vengeance;
			self.ToggleDamage();
			self.BlockAoE();
			ConROWindow:SetAlpha(ConRO.db.profile.transparencyWindow);
			ConRODefenseWindow:SetAlpha(ConRO.db.profile.transparencyWindow);
...
function ConRO:EnableDefenseModule(mode)
	...
	if mode == 1 then
		if ConRO.db.profile._Spec_1_Enabled then
			self.NextDef = ConRO.Evoker.HavocDef;
		else
	...
	if mode == 2 then
		if ConRO.db.profile._Spec_2_Enabled then
			self.NextDef = ConRO.Evoker.VengeanceDef;
		else
			self.NextDef = ConRO.Evoker.Disabled;
		end
	end;
...
function ConRO.Evoker.Havoc(_, timeShift, currentSpell, gcd, tChosen, pvpChosen)
--Info
	local _Player_Level																					= UnitLevel("player");
	local _Player_Percent_Health 																		= ConRO:PercentHealth('player');
	local _is_PvP																						= ConRO:IsPvP();
...
function ConRO.Evoker.HavocDef(_, timeShift, currentSpell, gcd, tChosen, pvpChosen)
--Info
	local _Player_Level																					= UnitLevel("player");
	local _Player_Percent_Health 																		= ConRO:PercentHealth('player');
	local _is_PvP																						= ConRO:IsPvP();
...
function ConRO.Evoker.Vengeance(_, timeShift, currentSpell, gcd, tChosen, pvpChosen)
--Info
	local _Player_Level																					= UnitLevel("player");
	local _Player_Percent_Health 																		= ConRO:PercentHealth('player');
	local _is_PvP																						= ConRO:IsPvP();
...
function ConRO.Evoker.VengeanceDef(_, timeShift, currentSpell, gcd, tChosen, pvpChosen)
--Info
	local _Player_Level																					= UnitLevel("player");
	local _Player_Percent_Health 																		= ConRO:PercentHealth('player');
	local _is_PvP																						= ConRO:IsPvP();
